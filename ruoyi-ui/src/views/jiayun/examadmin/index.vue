<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="考试科目ID" prop="subjectId">
        <el-input
          v-model="queryParams.subjectId"
          placeholder="请输入考试科目ID"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
...
<!--      </el-form-item>-->
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['jiayun:examadmin:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['jiayun:examadmin:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['jiayun:examadmin:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['jiayun:examadmin:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="examadminList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="考试ID" align="center" prop="id" />
      <el-table-column label="学员ID" align="center" prop="studentId" />
      <el-table-column label="考试科目ID" align="center" prop="subjectId" />
      <el-table-column label="考试标题" align="center" prop="title" />
      <el-table-column label="考试地点" align="center" prop="examPlace" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="考试科目ID" prop="subjectId">
          <el-input v-model="form.subjectId" placeholder="请输入考试科目ID" />
        </el-form-item>
        <el-form-item label="考试标题" prop="title">
          <el-input v-model="form.title" placeholder="请输入考试标题" />
        </el-form-item>
        <el-form-item label="考试地点" prop="examPlace">
          <el-input v-model="form.examPlace" placeholder="请输入考试地点" />
        </el-form-item>
        <el-form-item label="考试时间" prop="examTime">
          <el-date-picker clearable
...
<script>
import { listExamadmin, getExamadmin, delExamadmin, addExamadmin, updateExamadmin } from "@/api/jiayun/examadmin"
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        subjectId: [
          { required: true, message: "考试科目ID不能为空", trigger: "blur" }
        ],
        title: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listExamadmin(this.queryParams).then(response => {
        this.examadminList = response.rows
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateExamadmin(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addExamadmin(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('jiayun/examadmin/export', {
        ...this.queryParams
      }, `examadmin_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
